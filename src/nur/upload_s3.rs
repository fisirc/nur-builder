...
pub async fn upload_to_s3(bucket: &str, key: &str, file_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let access_key = env::var("AWS_ACCESS_KEY_ID")
        .map_err(|_| "Missing AWS_ACCESS_KEY_ID in .env")?;
    let secret_key = env::var("AWS_SECRET_ACCESS_KEY")
        .map_err(|_| "Missing AWS_SECRET_ACCESS_KEY in .env")?;
